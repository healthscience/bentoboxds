<template>
  <div class="earth-grid-container" @mousemove="handleGlobalMouseMove">
    ...
    >
      ...
      <div class="home-anchor" @click="returnToGenesis">
...
const lensPos = ref({ x: 50, y: 50 }); // Percentages for the mask
...
const returnToGenesis = () => {
  ...
  lensPos.value = { x: 50, y: 50 };
};

const handleGlobalMouseMove = (e) => {
  // Update lens position for the CSS Mask
  const rect = e.currentTarget.getBoundingClientRect();
  lensPos.value.x = ((e.clientX - rect.left) / rect.width) * 100;
  lensPos.value.y = ((e.clientY - rect.top) / rect.height) * 100;
};
...
const lensStyle = computed(() => ({
  'mask-image': `radial-gradient(circle 250px at ${lensPos.value.x}% ${lensPos.value.y}%, black 100%, transparent 100%)`,
  '-webkit-mask-image': `radial-gradient(circle 250px at ${lensPos.value.x}% ${lensPos.value.y}%, black 100%, transparent 100%)`
}));
</script>
...
.portal-lens.drawing-mode {
  pointer-events: auto; /* Intercept clicks to draw */
}
