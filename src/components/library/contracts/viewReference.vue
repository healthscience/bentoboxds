<template>
  <div id="cnrl-view"> REFERENCE VIEWER {{ referenceData }}
    <component v-bind:is="refTypeLive">
    ...
    <ul v-for="pb in boardPeerData" :key="pb.id" >
       gpm--{{ viewerType }} {{ refTypeLive }}
      <li class="ref-wrapper">
    ...
    <ul v-for="pcd in referencePeerData" :key="pcd.is">
      <li class="ref-wrapper"  v-if="viewerType === 'module-view'">
        Peer Module Library
        pm--{{ viewerType }} {{ refTypeLive }}
        <component v-bind:is="viewMapper()" v-if="refTypeLive === 'peer-modules'">
          <template v-slot:header>
          <div v-if="pcd.value">
...
import { ref, computed } from 'vue'
  ...
  const referenceData = computed(() => {
    ...
    return storeLibrary.publicLibrary
  })
